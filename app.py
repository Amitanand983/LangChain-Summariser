...
MAX_TOKENS = 1024  # Maximum token limit for BART

# Embedder for VectorStore
embeddings = HuggingFaceEmbeddings()
def split_into_chunks(text, max_tokens=MAX_TOKENS):
    """Split text into chunks of max_tokens length, ensuring no chunk exceeds max_tokens."""
    # Tokenize the text and split it based on token count
    ...
    for i in range(0, len(tokens), max_tokens):
        ...
        chunked_texts.append(tokenizer.decode(chunk, skip_special_tokens=True))
...
if st.session_state.vectorstore:
    ...
    if user_question:
        with st.spinner("Thinking..."):
            # Define RetrievalQA chain with increased k
            qa_chain = RetrievalQA.from_chain_type(
                llm=llm,
                chain_type="stuff",
                retriever=st.session_state.vectorstore.as_retriever(k=5),  # Retrieve more documents (increase k)
            ...
            if sources:
                ...
                for source in sources:
                    # Display the source file name or URL
                    st.write(f"- {source.metadata.get('source', document_file.name if document_file else generic_url if generic_url else 'Unknown Source')}")
